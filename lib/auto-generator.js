...
class AutoGenerator {
    ...
    makeHeaderTemplate({ importSequelize } = {}) {
        let header = '';
        ...
        fieldAttrs.forEach(attr => {
            ...
            if (attr === 'foreignKey') {
                if (foreignKey && foreignKey.isForeignKey) {
                    ...
                    str += space[3] + '},\n';
                    str += space[3] + 'onUpdate: \'' + fieldObj[attr].rule_update + '\',\n';
                    str += space[3] + 'onDelete: \'' + fieldObj[attr].rule_delete + '\'';
                }
                else {
                    return true;
                }
                ...
                if (fieldObj[attr] === true) { // ha primary key és foreign key egyszerre, akkor ezt hozzáadni
